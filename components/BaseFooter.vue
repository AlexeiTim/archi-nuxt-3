...
<template>
  <footer
    class="flex flex-col-reverse items-center lg:items-start gap-4 lg:gap-[177px] md:flex md:flex-row py-5 lg:py-[61px]"
  >
    ...
    <div class="flex gap-[20.5px]">
      <div
        @click="emits('change-slide', -1)"
        class="cursor-pointer hover:scale-150 transition flex items-center"
      >
        <IconArrowLeft />
      </div>
      <div v-if="slides" class="flex gap-[13px] items-center">
        ...
        <div class="w-[200px] h-[1px] bg-dark_green bg-opacity-38 relative">
          <div
            class="bg-dark_green h-[1px] transition absolute top-0 z-10"
            :style="{
              width: `${Math.floor(
